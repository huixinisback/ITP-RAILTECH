...
from utils.ocr_preprocessing import compute_mileage_roi, recognize_digits
...
while True:
    ...
    if points is not None:
        ...
        roi_x1, roi_y1, roi_x2, roi_y2 = compute_mileage_roi(
    ...
    if current_time - last_scan_time >= scan_interval:
        ...
        if text_roi is not None and text_roi.size > 0:
            result, thresh = recognize_digits(text_roi, return_processed=True)
            mileage_text = result.mileage
        ...
        if last_qr_data in VALID_TRAINS and mileage_text:
            ...
            if result_key != last_sent_result:
                payload = {
                    ...
                    "ocr_confidence": ocr_confidence or min(0.99, 0.70 + len(mileage_text) * 0.03),
                    "qr_confidence": 0.98,
                }
    ...
    cv2.putText(
        frame,
        f"QR: {last_qr_data if last_qr_data else 'Scanning...'}",
        (20, 40),
    ...
    cv2.putText(
        frame,
        "Auto-scan every 2 sec | Press Q to quit",
        (20, 80),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (255, 255, 0),
